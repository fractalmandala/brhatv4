...
export async function odishaItin(){
	const { data, error } = await supabase
	.from('brhat-anveshi')
	.select()
	.eq('type','itinerary')
...
export async function odishaTemps(){
	const { data, error } = await supabase
	.from('brhat-anveshi')
	.select()
	.eq('type','temple')
...
.imagecontainer-hero { background-image: url('/images/anveshi/keyodismal.webp')}
...
.imagecontainer-col { width: 100%; background-size: cover; background-position: center center; }
