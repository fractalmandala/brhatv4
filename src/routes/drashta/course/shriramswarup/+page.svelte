...
<div class="container-wd-sidebar pad-a">
  ...
  <div class="area-main">
    <div class="imagebox back-image" style="background-image: url('/images/drashta/ramswarup.webp')">
			<div class="pagetitle">
			<h1>Introduction to <br>Shri Ram Swarup</h1>
			</div>
		</div>
    <div class="bodybox">
			{#if panel1}
			...
			{#if panel2}
				...
				{:then data}
					{#each data as item}
						<div class="boxc wide75 card content">
							<h6>{item.name}</h6>
							<p>{item.content}</p>
						</div>
					{/each}
			...
			{#if panel3}
				<div class="bodybody" in:fly="{{ delay: 400, duration: 200, x:500, y: 0, opacity: 0, easing: quintOut}}" out:fly="{{ delay: 0, duration: 300, x:-500, y: 0, opacity: 0, easing: quintOut}}">
					...
					{:then data}
					{#each data as item}
					<div class="boxc card wide75 takeaways">
						<h6>{item.name}</h6>
